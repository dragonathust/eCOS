...
static int find_entry(jffs2_dirsearch * ds)
{
	...
	D2(printk("find_entry for name = %s\n", ds->path));
	d = jffs2_lookup(dir, name, namelen);
	D2(printk("find_entry got dir = %x\n", d));
...
static int jffs2_mount(cyg_fstab_entry * fste, cyg_mtab_entry * mte)
{
	...
	if (jffs2_sb == NULL) {
		...
		D2(printk("jffs2_mount erasing pending blocks\n"));
#ifdef CYGOPT_FS_JFFS2_WRITE
...
void jffs2_iput(struct _inode *i)
{
 ...
 recurse:
	if (!i) {
		printk("jffs2_iput() called with NULL inode\n");
		// and let it fault... 
	}
