			  ...
			  size_t * return_size, unsigned char *write_buffer)
{
	...
	//D2(printk("FLASH READ\n"));
	//D2(printk("read address = %x\n", CYGNUM_FS_JFFS2_BASE_ADDRESS + read_buffer_offset));
	//D2(printk("write address = %x\n", write_buffer));
	//D2(printk("size = %x\n", size));
			   ...
			   struct jffs2_eraseblock * jeb)
{
	...
	//        D2(printk("FLASH ERASE ENABLED!!!\n"));
	//        D2(printk("erase address = %x\n", CYGNUM_FS_JFFS2_BASE_ADDRESS + jeb->offset));
	//        D2(printk("size = %x\n", c->sector_size));
